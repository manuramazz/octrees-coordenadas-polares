/*
 * Second version the octree header, trying to generalise the structure for both pointer and linear based octrees
 * 
...
class OctreeV2
{
	...
	// Return the octant of the given index (in regular order from 0 to 7)
	...
	// Build octree with a given radius
	OctreeV2(const Point& center, float radius);
	...
	std::vector<std::reference_wrapper<const OctreeV2>> getOctants() const {
    	std::vector<std::reference_wrapper<const OctreeV2>> octants;

		for (int index = 0; index < OCTANTS_PER_NODE; ++index) {
			auto octant = getOctant(index);
			if (octant) {
				octants.emplace_back(std::cref(*octant));
			}
		}
		return octants;
	}
	...
	*/
	...
	[[nodiscard]] const OctreeV2* findOctant(const Lpoint* p) const;
	...
	// Builds the octree from a given set of points
