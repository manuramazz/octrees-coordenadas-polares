...
FULL_ALGOS="neighbors,neighborsV2,neighborsPtr,neighborsStruct,neighborsPCLKD,neighborsUnibn"
set -e
mkdir -p "$FOLDER"
...
# # Full cloud searches
./build/rule-based-classifier-cpp --kernels "$KERNELS_3D" -i "data/semantic3d/bildstein_station1_xyz_intensity_rgb.las" -o "$FOLDER/full" -r "0.01,0.05,0.1" -s "all" --sequential --repeats 1 --no-warmup -a "neighborsUnibn"
./build/rule-based-classifier-cpp --kernels "sphere" -i "data/dales_las/test/5080_54400.las" -o "$FOLDER/full" -r "15.0" -s "all" --sequential --repeats 1 --no-warmup -a "neighborsUnibn"
./build/rule-based-classifier-cpp --kernels "sphere" -i "data/paris_lille/Lille_0.las" -o "$FOLDER/full" -r "3.0" -s "all" --sequential --repeats 1 --no-warmup -a "neighborsUnibn"
./build/rule-based-classifier-cpp --kernels "sphere" -i "data/paris_lille/Paris_Luxembourg_6.las" -o "$FOLDER/full" -r "3.0" -s "all" --sequential --repeats 1 --no-warmup -a "neighborsUnibn"
./build/rule-based-classifier-cpp --kernels "sphere" -i "data/semantic3d/sg27_station8_intensity_rgb.las" -o "$FOLDER/full" -r "0.01,0.025,0.05" -s "all" --sequential --repeats 1 --no-warmup -a "neighborsUnibn"
./build/rule-based-classifier-cpp --kernels "sphere" -i "data/speulderbos/Speulderbos_2017_TLS.las" -o "$FOLDER/full" -r "0.25" -s "all" --sequential --repeats 1 --no-warmup -a "neighborsUnibn"
...
./build/rule-based-classifier-cpp --kernels "sphere" -i "data/paris_lille/Lille_0.las" -o "$FOLDER/parallel_subset" -r "0.25,0.5,1.0,2.0" -s "$N_SEARCHES" --repeats 5 -a "neighborsPtr,neighborsStruct" --num-threads "$NUM_THREADS"
